...
int main(){
    ...
    for (int i=0; i<NUM_DEPOSIT_THREADS; i++){
        data[i].addition = 200;
        data[i].threadid = i;
        pthread_create(&depositt[i], NULL, add_balance, &data[i]);
    }
    
    for (int i=0; i<NUM_WITHDRAW_THREADS; i++){
        data[i + NUM_DEPOSIT_THREADS].addition = -100;
        data[i + NUM_DEPOSIT_THREADS].threadid = i;
        pthread_create(&withdrawt[i], NULL, add_balance, &data[i + NUM_DEPOSIT_THREADS]);
    }
