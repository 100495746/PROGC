...
int main() {
    ...
    //waits until we have pressed enter
    while ((c = getchar()) != '\n' && c != EOF) {}
    ...
    */
    ...
    if(strcmp(option, "age") == 0){
        ...
        scanf("%d", &student[student_number-1].age);

    } else if (strcmp(option, "grade") == 0) {
         printf("Enter new grade: ");
        scanf("%f", &student[student_number-1].grade);
       
    } else{
        printf("Invalid option");
    }

    // Move the file pointer to the position of the student to modify
    fseek(file, (student_number - 1) * sizeof(struct Student), SEEK_SET);

    fwrite(&student[student_number - 1], sizeof(struct Student), 1, file);
